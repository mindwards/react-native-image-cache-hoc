...
export class FileSystem {
  ...
  constructor(cachePruneTriggerLimit = null, fileDirName = null, fetchParamsCallback = url => ({ url, headers: {} })) {
    this.os = Platform.OS;
    ...
    this.fetchParamsCallback = fetchParamsCallback;
  }
  ...
  async getExtensionFromContentTypeHeader(url) {
    ...
    try{
      ...
      const { url: processedUrl, headers } = this.fetchParamsCallback(url);
  ...
  async fetchFile(url, permanent = false, fileName = null, clobber = false) {
    ...
    try {
      ...
      result = await RNFetchBlob
        ...
        .fetch('GET', processedUrl, headers);
 ...
 * @returns {FileSystem}
 */
export default function FileSystemFactory(cachePruneTriggerLimit = null, fileDirName = null, fetchParamsCallback = null) {
  if (!(this instanceof FileSystem)) {
    return new FileSystem(cachePruneTriggerLimit, fileDirName, fetchParamsCallback);
  }
}
